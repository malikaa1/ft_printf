...
def launch_tests():
    result1 = subprocess.run("{} -o tests_ftprintf main_ftprintf.c -L{} -lftprintf".format(COMP, PATH[:-1]), shell=True)
    result2 = subprocess.run("{} -o tests_printf main_printf.c -L{} -lftprintf".format(COMP, PATH[:-1]), shell=True)
